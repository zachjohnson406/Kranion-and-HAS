function [] = subPlotWithTemp(img1, img2, orientation, ax, voxel_size)
    if nargin < 4
    ...
    % Overlay temperature data with colormap 'hot' using proper coordinates
    hImg = imagesc(ax, x_coords, y_coords, img2, [minscale 30]);
    alphaData = ones(size(img2));
    alphaData(img2 <= 0) = 0; % transparent where temperature <= 0
    hImg.AlphaData = 0.7;
    colormap(ax, 'hot'); % only for the overlay
    
    % Set axis properties
    axis(ax, 'image');
    xlabel(ax, 'R/L (mm)','FontSize',13);
    ylabel(ax, 'P/A (mm)','FontSize',13);
    title(ax, 'Simulation','FontSize',17);
    
    % Calculate extents for tick placement
