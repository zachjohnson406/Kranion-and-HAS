% Loads Kranion export infomation from xml and saves into structures 
% compatable with HAS. 
% 
% @OUTPUTS
%   loc: Matrix corresponding to the locations of transduce elements.
%   Vhas: HAS model. 
%   FocCTijk: Indicies of geometric focus of the transducer in the Model. 

% @INPUTS
%   KRXfile: Kranrion export file.
%   KRXpath: Path to the Kranion export file in use. 
%
%
...
function [loc, Vhas,NumSon, toKranion] = loadKranionScene(KRXfile, KRXpath)
        ...
        save([KRXpath KRXsubfolder filesep 'CT_Volume.mat'], 'unTransform', 'Size');
    ...
    %        %%%%%%% NOT NEEDED (because I have per-element phase) %%%%%%%% Sonication.FocusSteering = ; %FocusSteering
            ...
            FocCT = inv(mm)*Foc; %% FocCT is in millimeters 
            
            xscal = x(1:length(Vhas(1,:,1)))/Scale(1,1);
            yscal = y(1:length(Vhas(:,1,1)))/Scale(2,2);
            zscal = z(1:length(Vhas(1,1,:)))/Scale(3,3);
           
            FocCTijk(1) = find(xscal > FocCT(1),1);
            FocCTijk(2) = find(yscal > FocCT(2),1);
            FocCTijk(3) = find(zscal > FocCT(3),1);

            save(['SonicationParameters',num2str(j+1) ,'.mat'], 'FocCTijk','Sonication');
